...
const ChatView = ({ chat, conversation, onBack }) => {
  ...
  const exportChat = (asPhone = false) => {
    ...
    const sendAsNode = node.querySelector('.send-as-section');

    if (sendAsNode) sendAsNode.style.display = 'none';
    ...
    htmlToImage.toPng(node, exportOptions)
      ...
      .finally(() => {
        if (sendAsNode) sendAsNode.style.display = 'flex';
  ...
  return (
    <div ref={exportRef} className="flex flex-col h-full bg-primary-bg-light dark:bg-primary-bg-dark">
      {/* Header */}
      ...
      {/* Message Area */}
      <div className="flex-grow p-4 overflow-y-auto bg-secondary-bg-light dark:bg-secondary-bg-dark">
